...
class Network(object):
    ...
    def predict(self, sample):
        ...
        p=map(lambda node: node.output, self.layers[-1].nodes[:-1])
        return list(p)
...
class Normalizer(object):
    ...
    def norm(self, number):
        a=map(lambda m: 0.9 if number & m else 0.1, self.mask)
        return list(a)
...
def train(network):
    ...
    network.train(labels, data_set, 0.3, 2)
